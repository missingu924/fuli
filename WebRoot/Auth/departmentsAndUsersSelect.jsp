...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
 	...
 	if (level2Department.getCh().size() >= 0)
 			{
 %> <a href="#"
										onclick="selectSubject('<%=SystemConstant.SUBJECT_TYPE_DEPARTMENT%>','<%=level2Department.getK()%>','<%=level2Department.getV()%>','<%=level2Department.getK()%>','<%=level2Department.getV()%>','<%=level1Department.getV()%>','<%=level2Department.getCh().size() %>')">
											<%
												}
								...
								<td align="left">
									<table border="0" cellpadding="0" cellspacing="0" width="100%">
										...
										<td width="40px" style="padding:2px">
											...
											<a href="#"
												onclick="selectSubject('<%=SystemConstant.SUBJECT_TYPE_PERSON%>','<%=user.getK()%>','<%=user.getV()%>','<%=level2Department.getK()%>','<%=level2Department.getV()%>','<%=level1Department.getV()%>','1')"><%=user.getV()%></a>
										</td>
										<%
